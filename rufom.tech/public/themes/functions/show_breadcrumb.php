...
echo '<div class="breadcrumb" xmlns:v="http://rdf.data-vocabulary.org/#" itemscope itemtype="https://schema.org/BreadcrumbList">';

if ($this->diafan->_site->id != 1)
{
	echo  '<span typeof="v:Breadcrumb" itemprop="itemListElement" itemscope
      itemtype="https://schema.org/ListItem"><a href="'.BASE_PATH_HREF.'" rel="v:url" property="v:title" itemprop="item">'.$this->diafan->_('Главная').'</a> '.$separator.' </span>';
}
if ($this->diafan->_site->parent_id)
{
	...
	{
		echo '<span typeof="v:Breadcrumb" itemprop="itemListElement" itemscope
      itemtype="https://schema.org/ListItem"><a href="'.BASE_PATH_HREF.$row["link"].'" rel="v:url" property="v:title" itemprop="item">'.$this->diafan->_useradmin->get($row["name"], 'name', $row["id"], 'site', _LANG).'</a> '.$separator.' </span>';
	}
}
if ($this->diafan->_site->breadcrumb)
{
	foreach ($this->diafan->_site->breadcrumb as $breadcrumb)
	{
		...
		echo '<span typeof="v:Breadcrumb" itemprop="itemListElement" itemscope
      itemtype="https://schema.org/ListItem"><a href="'.BASE_PATH_HREF.$breadcrumb["link"].'" rel="v:url" property="v:title" itemprop="item">'.$breadcrumb["name"].'</a> '.$separator.' </span>';
	}
}
if($current)
{
	...
	echo '<span typeof="v:Breadcrumb" itemprop="itemListElement" itemscope
      itemtype="https://schema.org/ListItem"><span>'.$name.'</span></span>';
}
echo '</div>';
