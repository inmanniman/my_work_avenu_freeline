...
class Theme_admin extends Diafan
{
	...
	public function show_js()
	{
		...
		<script type="text/javascript" defer src="'.BASE_PATH.File::compress(Custom::path('adm/js/admin.js'), 'js').'" charset="UTF-8"></script>';
		if($this->diafan->is_action("edit"))
		{
		...
		{
			echo '<script type="text/javascript" defer src="'.BASE_PATH.File::compress(Custom::path('adm/js/admin.show.js'), 'js').'" charset="UTF-8"></script>
			<script type="text/javascript" defer src="'.BASE_PATH.File::compress(Custom::path('adm/js/admin.move.js'), 'js').'" charset="UTF-8"></script>';
		}
		if ($this->diafan->is_action("edit"))
		{
			echo '<script defer src="'.BASE_PATH.Custom::path('js/jquery.fancybox.min.js').'" type="text/javascript" charset="utf-8"></script>';
		}
		...
		if($this->diafan->_admin->rewrite && $this->diafan->_admin->module)
		{
			...
			{
				$this->diafan->_admin->js_view[] = $file.'.js'; //echo '<script type="text/javascript" defer src="'.BASE_PATH.File::compress(Custom::path($file.'.js'), 'js').'"></script>';
			}
			if($this->diafan->is_action("edit") && Custom::exists($file.'.edit.js'))
			{
				$this->diafan->_admin->js_view[] = $file.'.edit.js'; //echo '<script type="text/javascript" defer src="'.BASE_PATH.File::compress(Custom::path($file.'.edit.js'), 'js').'"></script>';
			}
		...
		foreach($this->diafan->_admin->js_view as $path)
		{
			...
			echo '<script type="text/javascript" defer src="'.$path.'"></script>';
		}
