			...
			if($price_arr = $this->diafan->values("price_arr"))
			{
                ...
                foreach ($price_arr as $price){
                    foreach ($this->cache["multiple_params"] as $row){
                        $sort[$row['id']][] = array_search($price['param'][$row['id']], array_column($row['options'], 'id'));
                    }
                }
                $first_sort = array_shift($sort);
                $sort[] = &$price_arr;
                array_multisort($first_sort, SORT_NUMERIC, ...$sort);
                ...
                foreach ($price_arr as $price)
				{
					...
					{
						echo '<img src="'.BASE_PATH.USERFILES.'/small/'.($price["image_rel"]["folder_num"] ? $price["image_rel"]["folder_num"].'/' : '').$price["image_rel"]["name"].'" loading="lazy">';
					}
