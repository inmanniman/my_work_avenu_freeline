...
$(document).on('click', "[action=one_click]", function() {
...
$(document).on('click', ".js_cart_one_click_form :button, .cart_one_click_form :button", function(){
	...
	$('.js_shop_form_param input, .js_shop_form_param select, .shop_form_param input, .shop_form_param select, .js_shop_additional_cost input[type="checkbox"]:checked', 'form[one_click=true]').each(function(){
...
$('.js_get_value').each(function(){
	var self = $(this);
	var field = $('[name="' + self.data('from') + '"]');
	var update = function(){
		self.html($(':selected', this).html());
	};
	$(field).change(update);
	update.call(field);
});
...
function select_param_price(th)
{
	...
	$(".js_shop_depend_param", th).each(function(){
		...
		if (v_param_code)
		{
			...
			if(selected_hide && first_visible){
				$(this).val(first_visible.value);
				$(this).trigger('change');
			}
		}
