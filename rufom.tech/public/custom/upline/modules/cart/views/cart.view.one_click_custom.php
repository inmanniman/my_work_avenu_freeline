...
if (! empty($result["rows_param"]))
{
	foreach ($result["rows_param"] as $row)
	{
		...
		switch ($row["type"])
		{
			...
			case 'text':
				echo '<div class="field_wrap"><div class="infofield">'.$row["name"].($row["required"] ? '<span style="color:red;">*</span>' : '').':</div>
				<input type="text" name="p'.$row["id"].'" class="form-control" value="'.$value.'"></div>';
				break;

			case "phone":
				echo '<div class="field_wrap"><div class="infofield">'.$row["name"].($row["required"] ? '<span style="color:red;">*</span>' : '').':</div>
				<input type="tel" name="p'.$row["id"].'" class="form-control" value="'.$value.'"></div>';
				break;
...
echo '<div class="btn-wrap"><button type="button" class="btn">Оставить заявку</button></div>';
