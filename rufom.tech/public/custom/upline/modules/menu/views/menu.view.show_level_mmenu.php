...
foreach ($result["rows"][$result["parent_id"]] as $row)
{
	...
	elseif ($row["active_child"])
	{
		...
		echo ' <li class="menu-item has-children active">';
	}
	elseif ($row["children"])
	{
	...
	)
	{
		if ($row["othurl"])
		{
			echo '<a href="'.$row["othurl"].'"'.$row["attributes"].' class="menu-link '
			.(! empty($row["active"]) || ! empty($row["active_child"]) ? 'active' : '')
			.'">';
		}
		else
		{
			echo '<a href="'.BASE_PATH_HREF.$row["link"].'"'.$row["attributes"].' class="menu-link ' .(! empty($row["active"]) || ! empty($row["active_child"]) ? ' active' : '')
			.'">';
		}
	}
