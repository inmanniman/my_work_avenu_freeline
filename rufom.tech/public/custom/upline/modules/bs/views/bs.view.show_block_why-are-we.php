...
echo '<section class="why-are-we container">';
foreach ($result as $i => $row)
{
	if (! empty($row['html']) || ! empty($row['image']) || ! empty($row['swf']))
	{
        ...
        if (! empty($row['image']))
        {
            echo '<div class="why-are-we__wrapper-img">';
            echo '<img width="481" height="460" class="why-are-we__img" src="'.BASE_PATH.USERFILES.'/bs/'.$row['image'].'" alt="'.(! empty($row['alt']) ? $row['alt'] : '').'" title="'.(! empty($row['title']) ? $row['title'] : '').'">';
            echo '</div>';
        }
        //вывод баннера в виде html разметки
        if (! empty($row['text']))
        {
            echo '<div class="why-are-we__block">';
            echo '<h3 class="why-are-we__title base-title">' . $row['name'] . '</h3>';
            echo $row['text'];
            echo '</div>';
        }
	}
