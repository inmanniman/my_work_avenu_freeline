...
echo '<div class="top-slider owl-carousel">';
foreach ($result as $i => $row)
{
	if (! empty($row['html']) || ! empty($row['image']) || ! empty($row['swf']))
	{
		echo '<div class="item" style="background-image:url('.(! empty($row['image']) ? BASE_PATH.USERFILES.'/bs/'.$row['image'] : '').')">';
			echo '<div class="item-block">';
				...
				if (! empty($row['text']))
				{
					echo '<div class="item__title">' . $row['title'] . '</div>';
					echo '<div class="item__desc">' . $row['text'] . '</div>';
				}
				
				//вывод ссылки на баннер, если задана
				if (! empty($row['link']))
				{
					echo '<a href="'.$row['link'].'" class="btn js_bs_counter bs_counter button" rel="'.$row['id'].'" '.(! empty($row['target_blank']) ? 'target="_blank"' : '').'>'.$this->diafan->_("узнать подробнее").'</a>';
				}
