...
echo '<section class="rubber-gaskets">';
foreach ($result as $i => $row)
{
	if (! empty($row['html']) || ! empty($row['image']) || ! empty($row['swf']))
	{
				...
				if (! empty($row['text']))
				{
                    ...
                    echo '<h1 class="rubber-gaskets__title base-title">' . $row['title'] . '</h1>';
                    echo $row['text'];
				...
				if (! empty($row['image']))
				{
					 ...
					 echo '<img src="'.BASE_PATH.USERFILES.'/bs/'.$row['image'].'" alt="'.(! empty($row['alt']) ? $row['alt'] : '').'" title="'.(! empty($row['title']) ? $row['title'] : '').'" width="686" height="466" class="rubber-gaskets__img">';
					 echo '</div>';
				}
