...
//        placeholder="' . $row["name"] . '"
        ...
        if($row["id"] == 19 or $row["id"] == 20 or $row["id"] == 21){
            echo '<div><input id="id'.$row["id"].'" type="hidden" name="p' . $row["id"] . '" value="" class="form-control"></div>';
        }else{
		switch ($row["type"])
		{
			case 'title':
            ...
            case 'text':
                echo '<div><input id="id'.$row["id"].'" type="text" name="p' . $row["id"] . '" value="" class="form-control" placeholder="' . $row["name"] . '"></div>';
                break;

            case "email":
                echo '<div><input id="id'.$row["id"].'" type="email" name="p' . $row["id"] . '" value="" class="form-control" placeholder="' . $row["name"] . '"></div>';
                break;

            case "phone":
                echo '<div><input id="id'.$row["id"].'" type="tel" name="p' . $row["id"] . '" value="" class="form-control" placeholder="' . $row["name"] . '"></div>';
                break;

            case 'textarea':
                echo '<div><textarea id="id'.$row["id"].'" name="p' . $row["id"] . '" rows="5" class="form-control"  placeholder="' . $row["name"] . '"></textarea></div>';
                break;
			...
			case 'datetime':
				...
				echo '<div class="infofield">'.$row["name"].($row["required"] ? '<span style="color:red;">*</span>' : '').':</div>
					<input type="text" id="id'.$row["id"].'" name="p'.$row["id"].'" value="" class="timecalendar" showTime="'
					.($row["type"] == 'datetime'? 'true' : 'false').'">';
				break;

			case 'numtext':
				echo '<div class="infofield">'.$row["name"].($row["required"] ? '<span style="color:red;">*</span>' : '').':</div>
				<input type="number" id="id'.$row["id"].'" name="p'.$row["id"].'" value="">';
				break;

			case 'checkbox':
				echo '<input id="id'.$row["id"].'" name="p'.$row["id"].'" id="feedback_p'.$row["id"].'" value="1" type="checkbox" ><label for="feedback_p'.$row["id"].'">'.$row["name"].($row["required"] ? '<span style="color:red;">*</span>' : '').'</label>';
				break;
			...
			case 'radio':
				...
				{
					echo '<input id="id'.$row["id"].'" name="p'.$row["id"].'" type="radio" value="'.$select["id"].'" id="feedback_form_p'.$row["id"].'_'.$select["id"].'"> <label for="feedback_form_p'.$row["id"].'_'.$select["id"].'">'.$select["name"].'</label>';
				}
				break;
			...
			case 'multiple':
				...
				foreach ($row["select_array"] as $select)
				{
					echo '<input id="id'.$row["id"].'" name="p'.$row["id"].'[]" id="feedback_p'.$select["id"].'[]" value="'.$select["id"].'" type="checkbox"><label for="feedback_p'.$select["id"].'[]">'.$select["name"].'</label><br>';
				}
				break;
