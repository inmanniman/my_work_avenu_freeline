...
class Save_functions_admin extends Diafan
{
	...
	public function get_rewrite()
	{
		...
		$rewrite = preg_replace('/[^a-zA-Z0-9\-_абвгдеёжзийклмнопрстуфхцчшщъыьэюяАБВГДЕЁЖЗИЙКЛМНОПРСТУФХЦЧШЩЪЫЬЭЮЯ\/]+/u', '', $_POST["rewrite"]);

		$text = $_POST[$this->diafan->variable_list("name", "variable") ? $this->diafan->variable_list("name", "variable") : 'name'];
		if($this->diafan->_admin->module == 'faq')
		{
			$text = $_POST['anons'];
		}
		...
		$data = array(
			...
			'created' => (isset($_POST["created"]) ? $this->diafan->unixdate($_POST["created"]) : ''),
		);

		$this->diafan->_route->save($rewrite, $text, $element_id, $module_name, $element_type, $site_id, $cat_id, $parent_id, $add_parents, $change_children, $data);
		...
		foreach ($rows as $row)
		{
			...
			elseif (! empty($_POST['param'.$row["id"]]))
			{
				...
				switch($row["type"])
				{
					...
					case "numtext":
						$_POST['param'.$row["id"]] = $this->diafan->filter($_POST['param'.$row["id"]], 'float');
						break;
				}
			...
			case "multiple":
				...
				if ( ! empty($ids))
				{
					...
					if($del_ids)
					{
						...
						$columns = DB::columns($this->diafan->table.'_element');
						$value_name = isset($columns['value']) ? 'value' : 'value'.$this->diafan->_languages->site;

						DB::query("DELETE FROM {".$this->diafan->table."_element} WHERE param_id=%d AND ".$value_name." IN (%s)", $this->diafan->id, implode(",", $del_ids));

						$element_type = $this->diafan->element_type();

						$this->diafan->_route->delete($del_ids, $this->diafan->_admin->module, $element_type);

						DB::query("DELETE FROM {redirect} WHERE module_name='%s' AND element_id IN (%s) AND element_type='%s'", $this->diafan->_admin->module, implode(",", $del_ids), $element_type);
					}
				}
	...
	{
		$field = $this->diafan->variable_multilang('name') ? '[name]' : 'name';
