...
{
	echo "\n".'<link rel="stylesheet" href="'.BASE_PATH.File::compress('css/jquery.fancybox.min.css', 'css').'" type="text/css" media="screen" charset="utf-8">';
}
...
if (in_array('news', $this->diafan->installed_modules) && DB::query_result("SELECT id FROM {news} WHERE act"._LANG."='1'"))
{
	echo "\n".'<link rel="alternate" type="application/rss+xml" title="RSS" href="'.BASE_PATH.'news/rss/">';
}
if (in_array('clauses', $this->diafan->installed_modules) && DB::query_result("SELECT id FROM {clauses} WHERE act"._LANG."='1'"))
{
