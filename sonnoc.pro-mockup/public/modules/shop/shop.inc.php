...
class Shop_inc extends Diafan
{
	...
	public function price_calc($good_id = 0, $discount_id = 0, $currency_id = 0)
	{
		...
		if($discounts)
		{
			...
			// while($rows = DB::query_range_fetch_all("SELECT p.*, s.cat_id, s.brand_id FROM {shop_price} AS p INNER JOIN {shop} AS s ON p.good_id=s.id WHERE p.trash='0'".($good_id ? " AND p.good_id=".$good_id : '')." AND p.price_id=p.id AND p.id<=%d AND IsNull(p.old_price, '')='' AND s.no_buy='0'".($use_count_goods ? " AND p.count_goods>0" : ""), $safe_id, $polog, $nastr))
			{
				...
				foreach ($rows as $row)
				{
					//выбираем категории товара
					$cats = DB::query_fetch_value("SELECT cat_id FROM {shop_category_rel} WHERE element_id=%d", $row["good_id"], "cat_id");

					//производитель товара
					$brands = array();

					//если товару задан производитель 
					if ($row["brand_id"])
					{
						if ($cats)
						{
							//выбираем производителя с учётом категорий
							//производитель может быть привязан к катогориям, а может оказаться «свободным» (товар привязан к категории/ям, а производитель не привязан ни к какой/им)
							$brands = DB::query_fetch_value("SELECT DISTINCT(element_id) FROM {shop_brand_category_rel} WHERE element_id=%d AND cat_id IN (0,".implode(",", $cats).")", $row["brand_id"], "element_id");
						}
						else
						{
							//товару без категорий подходит всякий производитель
							$brands[] = $row["brand_id"];
						}
					}
	...
	/**
	 * Является ли товар элементом набора
	 *
	 * @param integer $good_id номер элемента набора
	 * @param integer $good_set_id номер набора
	 * @return boolean
	 */
	public function is_element_good_set($good_id, $good_set_id = 0)
	{
		$key = $good_set_id.'_'.$good_id;

		if (! isset($this->cache['is_element_good_set'][$key]))
		{
			$is_element = DB::query_result("SELECT id FROM {shop_set} WHERE set_element_id=%d".($good_set_id ? ' AND element_id=%d' : '')." AND trash='0'", $good_id, $good_set_id);

			$this->cache['is_element_good_set'][$key] = ! empty($is_element);
		}
		return $this->cache['is_element_good_set'][$key];
	}

	/**
	 * Запоминает номера наборов
	 *
	 * @param integer $good_id номер набора
	 * @return void
	 */
	public function prepare_elements_good_set($good_id)
	{
		$this->cache['prepare_elements_good_set'][$good_id] = $good_id;
	}

	/**
	 * Получает элементы набора
	 *
	 * @param integer $good_id номер набора
	 * @param integer $time текущее время, округленное до минут, в формате UNIX
	 * @return array
	 */
	public function get_elements_good_set($good_id, $time = 0)
	{
		if (! isset($this->cache['bind_good_set_element'][$good_id]))
		{
			$this->prepare_elements_good_set($good_id);

			$ids = array();
			foreach ($this->cache['prepare_elements_good_set'] as $id)
			{
				if (! isset($this->cache['bind_good_set_element'][$id]))
				{
					$this->cache['bind_good_set_element'][$id] = array();
			...
			if ($ids)
			{
				$binds = DB::query_fetch_all("SELECT element_id, set_element_id FROM {shop_set} WHERE element_id IN (%s) AND trash='0'", implode(',', $ids));

				$good_ids = array();
				foreach ($binds as $bind)
				{
					$this->cache['bind_good_set_element'][$bind['element_id']][$bind['set_element_id']] = $bind['set_element_id'];
	
					if (! isset($this->cache['elements_good_set'][$bind['set_element_id']]))
					{
						$this->cache['elements_good_set'][$bind['set_element_id']] = array();
	
						$good_ids[$bind['set_element_id']] = $bind['set_element_id'];
					}
					$this->cache['is_element_good_set']['0_'.$bind['set_element_id']] = true;
				}
				if ($good_ids)
				{
					if (empty($time))
					...
					$rows = DB::query_fetch_all(
						...
						." e.[measure_unit], e.hit, e.new, e.action, e.is_file, e.sort FROM {shop} AS e"
						...
						.($this->diafan->configmodules('hide_missing_goods', 'shop') ? " AND e.no_buy='0'" : ""), implode(',', $good_ids));
					...
					foreach ($rows as $row)
					{
						...
						$this->cache['elements_good_set'][$row['id']] = $row;
					}
				}
			}
		}
		$result = array();
		foreach ($this->cache['bind_good_set_element'][$good_id] as $id)
		{
			if (! empty($this->cache['elements_good_set'][$id]))
			{
				$result[] = $this->cache['elements_good_set'][$id];
			}
		}
		return $result;
	}
	...
	/**
	 * Запоминает номера элементов наборов
	 *
	 * @param integer $good_id номер элемента набора
	 * @return void
	 */
	public function prepare_goods_set($good_id)
	{
		$this->cache['prepare_goods_set'][$good_id] = $good_id;
	}

	/**
	 * Получает наборы для элемента
	 *
	 * @param integer $good_id номер элемента набора
	 * @param integer $time текущее время, округленное до минут, в формате UNIX
	 * @return array
	 */
	public function get_goods_set($good_id, $time = 0)
	{
		if (! isset($this->cache['bind_element_good_set'][$good_id]))
		{
			$this->prepare_goods_set($good_id);

			$ids = array();
			foreach ($this->cache['prepare_goods_set'] as $id)
			{
				if (! isset($this->cache['bind_element_good_set'][$id]))
				{
					$this->cache['bind_element_good_set'][$id] = array();
			...
			if ($ids)
			{
				$binds = DB::query_fetch_all("SELECT element_id, set_element_id FROM {shop_set} WHERE set_element_id IN (%s) AND trash='0'", implode(',', $ids));

				$good_ids = array();
				foreach ($binds as $bind)
				{
					$this->cache['bind_element_good_set'][$bind['set_element_id']][$bind['element_id']] = $bind['element_id'];
	
					if (! isset($this->cache['goods_set'][$bind['set_element_id']]))
					{
						$this->cache['goods_set'][$bind['set_element_id']] = array();
	
						$good_ids[$bind['element_id']] = $bind['element_id'];
					}
					$this->cache['is_element_good_set']['0_'.$bind['set_element_id']] = true;
				}
				if ($good_ids)
				{
					...
					foreach ($rows as $row)
					{
						...
						$this->cache['goods_set'][$row['id']] = $row;
					}
				}
			}
		}
		$result = array();
		foreach ($this->cache['bind_element_good_set'][$good_id] as $id)
		{
			if (! empty($this->cache['goods_set'][$id]))
			{
				$result[] = $this->cache['goods_set'][$id];
			}
		}
		return $result;
	}
