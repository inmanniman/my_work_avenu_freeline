			...
			foreach($sort_config['sort_fields_names'] as $key => $name)
			{
				...
				'<li class="setting-d__item">
					...
					<span class="setting-d__directions">';
						echo	
						'<a class="setting-d__direction setting-d__direction_asc'.(! $asc ? ' _active' : '').'" href="'.BASE_PATH_HREF.$asc.'" title="'.$this->diafan->_('По возрастанию', false).'">
							<span class="setting-d__icon icon-d fas fa-arrow-circle-up"></span>
						</a>';
						echo
						'<a class="setting-d__direction setting-d__direction_desc'.(! $desc ? ' _active' : '').'" href="'.BASE_PATH_HREF.$desc.'" title="'.$this->diafan->_('По убыванию', false).'">
