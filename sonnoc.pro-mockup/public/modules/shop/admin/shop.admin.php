...
class Shop_admin extends Frame_admin
{
	...
	public $variables = array (
		...
		'other_rows' => array (
			...
			'access' => array(
				'type' => 'function',
				'name' => 'Доступ',
				'help' => 'Если отметить опцию «Доступ только», товар увидят только авторизованные на сайте пользователи, отмеченных типов. Не авторизованные, в том числе поисковые роботы, увидят «403 Доступ запрещен» (администратору сайта).',
			),
			'map_no_show' => array(
				'type' => 'checkbox',
			...
			'yandex' => array(
				...
				'help' => 'Параметры, необходимые для формирования информации для системы «Яндекс Маркет» файлом YML. Параметр выводится, если в настройках модуля отмечена опция «Подключить Яндекс Маркет».',
				'depend' => 'show_yandex',
			),
			'hr9' => 'hr',
			'import_id' => array(
		...
		foreach($rows as $i => $row)
		{
			...
			if ($this->diafan->configmodules("use_count_goods"))
			{
				$text .= '<div class="count">
					...
					'.($item["measure_unit"] ? $item["measure_unit"] : $this->diafan->_('Шт')).'
				</div>';
			}
		}
		if(! empty($i))
	...
	public function edit_variable_good_set()
	{
		...
		if ( ! $this->diafan->is_new)
		{
			...
			{
				$rows_img = DB::query_fetch_key("SELECT element_id, id, name, folder_num FROM {images} WHERE element_id IN (%s) AND module_name='shop' AND element_type='element' AND param_id=0 AND trash='0' ORDER BY sort DESC", implode(",", $this->diafan->array_column($rows, "id")), "element_id");
			}
			foreach ($rows as $row)
			{
