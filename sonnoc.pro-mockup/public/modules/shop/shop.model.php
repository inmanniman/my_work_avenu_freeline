...
class Shop_model extends Model
{
	...
	public function id()
	{
		...
		if (! $this->result = $this->diafan->_cache->get($cache_meta, $this->diafan->_site->module))
		{
			...
			$row['goods_set'] = $this->diafan->_shop->get_goods_set($row['id'], $time);
	...
	public function show_search($site_ids, $cat_ids, $ajax)
	{
		...
		if (! $result = $this->diafan->_cache->get($cache_meta, "shop"))
		{
			...
			if ($this->diafan->configmodules("search_price", "shop", $site_ids[0]))
			{
				...
				$result["price"] = array(
					...
					"min" => floor(DB::query_result(
						...
						." AND p.date_start<=".$time." AND (p.date_finish=0 OR p.date_finish>=".$time.")"
			...
			foreach ($result["rows"] as $i => &$row)
			{
				...
				{
					$row["min"] = DB::query_result(
						"SELECT MIN(p.value".$this->diafan->_languages->site."+0) FROM {shop_param_element} AS p"
						." INNER JOIN {shop} AS s ON p.element_id=s.id AND s.[act]='1' AND s.trash='0'"
						.($this->diafan->configmodules('hide_missing_goods') ? " AND s.no_buy='0'" : "")
						.($cat_ids ? " INNER JOIN {shop_category_rel} AS c ON c.element_id=s.id AND c.cat_id IN (".implode(",", $cat_ids_down).")" : '')
						." WHERE p.param_id=%d",
						$row["id"]
					);
					$row["max"] = DB::query_result(
						"SELECT MAX(p.value".$this->diafan->_languages->site."+0) FROM {shop_param_element} AS p"
						." INNER JOIN {shop} AS s ON p.element_id=s.id AND s.[act]='1' AND s.trash='0'"
						.($this->diafan->configmodules('hide_missing_goods') ? " AND s.no_buy='0'" : "")
						.($cat_ids ? " INNER JOIN {shop_category_rel} AS c ON c.element_id=s.id AND c.cat_id IN (".implode(",", $cat_ids_down).")" : '')
						." WHERE p.param_id=%d",
		...
		{
			$result["article"]["value"] = ! empty($_REQUEST["a"]) ? $this->diafan->filter($_REQUEST, "string", "a") : '';
		}
	...
	public function elements(&$rows, $function = 'list', $images_config = '')
	{
		...
		foreach ($rows as &$row)
		{
			...
			$this->diafan->_shop->prepare_goods_set($row["id"]);
		}
		foreach ($rows as &$row)
		{
			$this->price($row);
		...
		foreach ($rows as &$row)
		{
			...
			$row['goods_set'] = $this->diafan->_shop->get_goods_set($row['id']);
		}
	...
	private function param(&$good, $function = "id")
	{
		...
		. " WHERE p.trash='0' AND p.site_id=%d"
		. " GROUP BY p.id ORDER BY p.sort ASC", $good["site_id"]
		);
	...
	private function where(&$where, &$where_param, &$values, &$getnav, &$group)
	{
		...
		foreach ($rows as $row)
		{
			...
			elseif($row["type"] == 'numtext' && (! empty($_REQUEST["p".$row["id"]."_2"]) || ! empty($_REQUEST["p".$row["id"]."_1"])))
			{
				...
				if(! $val1)
				{
					$where_param .= " LEFT JOIN {shop_param_element} AS pe".$row["id"]." ON pe".$row["id"].".element_id=s.id AND pe".$row["id"].".param_id='%d'"
						." AND pe".$row["id"].".trash='0'";

					$where .= " AND (pe".$row["id"].".value".$this->diafan->_languages->site."<=%f OR pe".$row["id"].".id IS NULL)";
					if ($val2)
					{
						$values[] = $val2;
