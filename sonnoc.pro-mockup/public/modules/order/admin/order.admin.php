...
class Order_admin extends Frame_admin
{
	 ...
	 * @return string
	 */
	public function get_filter_variable_delivery()
	{
		$value = (! empty($this->diafan->get_nav_params['filter_delivery']) ? $this->diafan->get_nav_params['filter_delivery'] : array());

		$count = DB::query_fetch_key_value("SELECT COUNT(e.id) AS count, e.delivery_id FROM {shop_order} as e".$this->diafan->join." WHERE e.trash='0'".preg_replace('/ AND e\.delivery_id IN \(.?\)/', '', $this->diafan->where)." GROUP BY e.delivery_id", "delivery_id", "count");

		$rows = DB::query_fetch_key_value("SELECT id, [name] FROM {shop_delivery} WHERE trash='0' ORDER BY sort ASC", 'id', 'name');
		...
		foreach($rows as $id => $name)
		{
			if(! $id)
				continue;
			$text .= '<p><input type="checkbox" name="filter_delivery[]" value="'.$id.'"'.(in_array($id, $value) ? ' checked' : '').' id="filter_delivery'.$id.'"><label for="filter_delivery'.$id.'">'.$name.' ('.(! empty($count[$id]) ? $count[$id] : 0).')</label></p>';
		}
	...
	/**
	 * Поиск по полю "Доставка"
	 *
	 * @param array $row информация о текущем поле
	 * @return mixed
	 */
	public function save_filter_variable_delivery($row)
	{
		if(! empty($_GET["filter_delivery"]) && is_array($_GET["filter_delivery"]))
		{
			$res = $this->diafan->filter($_GET["filter_delivery"], 'integer');
		}
		else
		{
			$res = array();
		...
		if($res)
		{
			$this->diafan->where .= " AND e.delivery_id IN (".implode(",", $res).")";
			$this->diafan->get_nav .= ($this->diafan->get_nav ? '&amp;' : '?') . 'filter_delivery[]='.implode("&amp;filter_delivery[]=", $res);
		}
		return $res;
	}
	...
	public function edit_variable_goods()
	{
		...
		if(! $this->diafan->is_new)
		{
			...
			foreach ($rows as $row)
			{
				...
				WHERE i.element_id=%d AND i.module_name='shop' AND i.element_type='element' AND i.param_id=0 AND i.trash='0'
