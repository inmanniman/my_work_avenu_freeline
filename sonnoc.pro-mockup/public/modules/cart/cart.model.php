...
class Cart_model extends Model
{
	...
	public function form_table($payment_id = 0)
	{
		...
		foreach ($this->result["rows"] as $i => &$row)
		{
			...
			// элементы набора
			if (! empty($row['good']['good_set']))
			{
				$row['elements_good_set'] = $this->diafan->_shop->get_elements_good_set($row['good']['id']);
			}
		}
	...
	private function prepare_form_data()
	{
		...
		foreach($this->result["rows"] as $row)
		{
			...
			if (! empty($row['good']['good_set']))
			{
				$this->diafan->_shop->prepare_elements_good_set($row['good']['id']);
			}
		}
