...
foreach ($result as $row)
{
	if (! empty($row['name']))
	{
		echo '<h1 class="banner__title">'. $row['name'] .'</h1>';
	}
	...
	if (! empty($row['image']))
	{
		echo '<img class="banner__img" src="'.BASE_PATH.USERFILES.'/bs/'.$row['image'].'" alt="'.(! empty($row['alt']) ? $row['alt'] : '').'" title="'.(! empty($row['title']) ? $row['title'] : '').'">';
	}
}
