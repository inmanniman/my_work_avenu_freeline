...
// начало уровня меню
echo '<div class="header__dropdown js-dropdown"><ul class="header__list">';
foreach ($result["rows"][$result["parent_id"]] as $row)
{
	...
	)
	{
		if ($row["othurl"])
		{
			echo '<a class="header__link link" href="'.$row["othurl"].'"'.$row["attributes"].'>';
		}
		else
		{
			echo '<a class="header__link link" href="'.BASE_PATH_HREF.$row["link"].'"'.$row["attributes"].'>';
		}
	}
