...
echo '<button class="base-btn mm-listitem__toggle">
        <svg class="mm-listitem__svg">
          <use href="/assets/sprite.svg#arrow"></use>
        </svg>
      </button>';
echo '<div class="m-menu__accordion NoPanel">
        <ul class="m-menu__accordion-list">';
// начало уровня меню
foreach ($result["rows"][$result["parent_id"]] as $row)
{
	if ($row["active"])
	{
		...
		echo '<li class="mm-listitem mm-listitem_accordion active">';
	}
	elseif ($row["active_child"])
	{
		// начало пункта меню для активного дочернего пункта
		echo ' <li class="mm-listitem mm-listitem_accordion parent active">';
	}
	elseif ($row["children"])
	{
		// начало пункта меню для элемента -родителя
		echo ' <li class="mm-listitem mm-listitem_accordion parent">';
	}
	else
	{
		// начало любого другого пункта меню
		echo '<li class="mm-listitem mm-listitem_accordion">';
	}
	...
	)
	{
		if ($row["othurl"])
		{
			echo '<a href="'.$row["othurl"].'" class="mm-listitem__text mm-listitem__text_accordion '.$row["attributes"].''
			.(! empty($row["active"]) || ! empty($row["active_child"]) ? ' active ' : '')
			.'">';
		}
		else
		{
			echo '<a href="'.BASE_PATH_HREF.$row["link"].'" class="mm-listitem__text mm-listitem__text_accordion '.$row["attributes"]
			.(! empty($row["active"]) || ! empty($row["active_child"]) ? ' active ' : '')
			.'">';
		}
	}
	...
	if ($result["show_all_level"] || $row["active_child"] || $row["active"])
	{
		...
		echo $this->get('show_level_mmenu_2', 'menu', $menu_data);
	}
