...
//    print_r($row);
	if ($row["active_child"])
    {
        ...
        echo ' <li class="header__item header__item_dropdown js-elem-with-dropdown">';
    }else
	if($row["active"])
	{
		// начало пункта меню для текущей страницы
		echo '<li class="header__item active">';
	}
	else
	if ($row["children"])
	{
	...
	)
	{
		if ($row["othurl"])
		{
			...
			.(! empty($row["active"]) || ! empty($row["active_child"]) ? ' class="header__link link"' : ' class="header__link link"')
			.'>';
		}
        if ($row["attributes"] == ' nolink')
        {
			//span > a
            echo '<a class="header__link link">';
        }
		else
		{
	...
	if ($result["show_all_level"] || $row["active_child"] || $row["active"] || $result["current_link"])
	{
