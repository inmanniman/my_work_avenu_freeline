...
class Shop_admin_config extends Frame_admin
{
	...
	public $variables = array (
		...
		'yandex_shop' => array (
			...
			'yandex_import' => array(
				'type' => 'checkbox',
				'name' => 'Подключить импорт из YML-файла',
		...
		'send_mails' => array (
			...
			'sendsmsnew' => array(
				...
				'help' => 'Возможность отправлять SMS пользователю при поступлении заказа. Параметр можно подключить, если в [Настройках модуля Уведомления](http://www.diafan.ru/dokument/full-manual/sysmodules/config/) настроены SMS-уведомления.',
			),
			...
			'sendsmschange' => array(
				...
				'help' => 'Возможность отправлять SMS пользователю при изменении статуса заказа. Параметр можно подключить, если в [Настройках модуля Уведомления](http://www.diafan.ru/dokument/full-manual/sysmodules/config/) настроены SMS-уведомления.',
			),
			...
			'sendsmsadmin' => array(
				...
				'help' => 'Возможность отправлять SMS администратору при поступлении заказа. Параметр можно подключить, если в [Настройках модуля Уведомления](http://www.diafan.ru/dokument/full-manual/sysmodules/config/) настроены SMS-уведомления.',
			),
			'sms_admin' => array(
				'type' => 'text',
	...
	public function edit_config_variable_yandex_import_path()
	{
		...
		{
			$value = $this->diafan->_('Укажите секретный ключ, чтобы увидеть ссылку.');
		}
		elseif(! $this->diafan->values('yandex_import_filename') && ! $this->diafan->values('yandex_import_url'))
		{
			$value = $this->diafan->_('Укажите название файла или URL-адрес, чтобы увидеть ссылку.');
		}
		else
		{
	...
	public function edit_config_variable_yandex_import_filename()
	{
		...
		<div class="unit depend_field" id="yandex_import_file" depend="yandex_import">
			<div class="infofield">'.$this->diafan->_('Или загрузите файл в папку tmp/yml').' <i class="tooltip fa fa-question-circle" title="'.$this->diafan->_('Выберите файл и нажмите кнопку «Сохранить», чтобы загрузить/обновить файл в папке *tmp/yml*.').'"></i></div>
