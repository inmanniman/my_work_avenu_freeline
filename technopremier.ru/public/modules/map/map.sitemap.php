...
class Map_sitemap extends Diafan
{
	...
	public function init()
	{
		...
		if(! $this->diafan->configmodules("full_index", "map") && ! $this->diafan->configmodules("all_token", "map"))
		{
			$this->diafan->_map->index_all();
			$this->diafan->redirect_js(BASE_PATH.'map/sitemap/wait/?time='.rand(0, 99999));
		}
		$this->get();
	}
	...
	private function generate()
	{
		...
		if(! $this->diafan->configmodules($type."_token", "map"))
		{
			return;
		}
		if(! empty($_GET["token"]) && $this->diafan->configmodules($type."_token", "map") == $_GET["token"])
		{
			if($type == 'all')
			{
				return $this->diafan->_map->index_all();
			}
			if(preg_match('/module_(.*)$/', $type, $m))
			{
				$this->diafan->_map->index_module($m[1]);
			}
		}
	}
	...
	private function wait()
	{
		...
		{
			$this->diafan->redirect_js(BASE_PATH.'sitemap.xml?rnd='.rand(0, 99999));
		}
		if($this->diafan->configmodules("all_token", "map"))
		{
			echo '<script language="javascript" type="text/javascript">
			setTimeout(\'window.location.href="'.BASE_PATH.'map/sitemap/wait/?rnd='.rand(0, 99999).'";\', 1000)</script>';
		}
	}
}
