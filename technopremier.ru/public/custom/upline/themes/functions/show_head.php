...
$act_news = DB::query_result("SELECT id FROM {news} WHERE act1='1' LIMIT 1");

if($act_news && in_array('news', $this->diafan->installed_modules))
{
	echo "\n".'<link rel="alternate" type="application/rss+xml" title="RSS" href="'.BASE_PATH.'news/rss/">';
}

$act_clauses = DB::query_result("SELECT id FROM {clauses} WHERE act1='1' LIMIT 1");

if($act_clauses && in_array('clauses', $this->diafan->installed_modules))
{
