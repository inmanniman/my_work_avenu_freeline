...
echo '<nav class="navigation__wrapper-content container" itemscope="" itemtype="http://schema.org/BreadcrumbList" id="breadcrumbs">';

if ($this->diafan->_site->id != 1)
{
    echo '<span class="breadcrumb-d" itemscope="" itemprop="itemListElement" itemtype="http://schema.org/ListItem"><a itemprop="item" href="'.BASE_PATH_HREF.'" class="navigation__link"><span itemprop="name">'.$this->diafan->_('Главная').'</span><meta itemprop="position" content="'.$position.'"></a> '.$separator.'</span> ';
    $position++;
}
if ($this->diafan->_site->parent_id)
{
    ...
    {
        echo '<span class="breadcrumb-d" itemscope="" itemprop="itemListElement" itemtype="http://schema.org/ListItem"><a itemprop="item" href="'.BASE_PATH_HREF.$row["link"].'" class="navigation__link"><span itemprop="name">'.$this->diafan->_useradmin->get($row["name"], 'name', $row["id"], 'site', _LANG).'</span><meta itemprop="position" content="'.$position.'"></a> '.$separator.'</span> ';
        $position++;
    }
}
if ($this->diafan->_site->breadcrumb)
{
    foreach ($this->diafan->_site->breadcrumb as $breadcrumb)
    {
        ...
        echo '<span class="breadcrumb-d" itemscope="" itemprop="itemListElement" itemtype="http://schema.org/ListItem"><a itemprop="item" href="'.BASE_PATH_HREF.$breadcrumb["link"].'" class="navigation__link"><span itemprop="name">'.$breadcrumb["name"].'</span><meta itemprop="position" content="'.$position.'"></a> '.$separator.'</span> ';
        $position++;
    }
}
if($current)
{
    ...
    echo '<span class="breadcrumb-d" itemscope="" itemprop="itemListElement" itemtype="http://schema.org/ListItem"><meta itemprop="item" content="'.BASE_PATH.$this->diafan->_site->rewrite.'"><span itemprop="name" class="navigation__link-active">'.$name.'</span><meta itemprop="position" content="'.$position.'"></span>';
}
echo '</nav>';
echo '</section>';
