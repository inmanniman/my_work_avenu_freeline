...
foreach ($result as $row)
{
	...
	if (! empty($row['image']))
	{
        echo '<div class="on-lease__wrapper-image">';
		echo '<img class="on-lease__img" loading="lazy" src="'.BASE_PATH.USERFILES.'/bs/'.$row['image'].'" alt="'.(! empty($row['alt']) ? $row['alt'] : '').'" title="'.(! empty($row['title']) ? $row['title'] : '').'">';
        echo '</div>';
	}
