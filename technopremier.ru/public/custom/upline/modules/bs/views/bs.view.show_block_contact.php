...
echo '<section class="contact__block">';
echo '<div class="contact">';
echo '<div class="contact__wrapper-content container">';
...
foreach ($result as $row)
{
    ...
    echo '<div class="contact__wrapper-content-text">';

    if (! empty($row['name']))
    {
        echo '<h2 class="contact__title">'.$row['name'].'<span class="contact__title_red">'.$row['alt'].'</span><br>'.$row['title'].'</h2>';
    }
    ...
    if (! empty($row['image']))
    {
        echo '<div class="contact__wrapper-img">';
            echo '<img class="contact__img" loading="lazy" src="'.BASE_PATH.USERFILES.'/bs/'.$row['image'].'" alt="'.(! empty($row['alt']) ? $row['alt'] : '').'">';
        echo '</div>';
    }
