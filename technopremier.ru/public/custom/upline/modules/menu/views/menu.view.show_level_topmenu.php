...
foreach ($result["rows"][$result["parent_id"]] as $row)
{
	...
	elseif ($row["active_child"])
	{
		...
		echo ' <li class="header__menu-item parent">';
	}
	elseif ($row["children"])
	{
	...
	{
//        <li class="header__menu-item"><a href="/" class="header__menu-txt-technic">ТЕХНИКА</a></li>
		if ($row["othurl"])
		{
			echo '<a href="'.$row["othurl"].'" class="header__menu-txt '.$row["attributes"]
			.'">';
		}
		else
		{
			echo '<a href="'.BASE_PATH_HREF.$row["link"].'" class="header__menu-txt '.$row["attributes"]
			.'">';
		}
	}
//    print_r($row);
