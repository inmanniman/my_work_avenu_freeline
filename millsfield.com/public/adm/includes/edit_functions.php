...
class Edit_functions_admin extends Diafan
{
	...
	public function edit_variable_rewrite()
	{
		...
		if (! $this->diafan->is_new)
		{
			...
			$row_redirect = DB::query_fetch_array("SELECT redirect, code FROM {redirect} WHERE module_name='%s' AND element_id=%d AND element_type='%s' AND trash='0' LIMIT 1", $this->diafan->_admin->module, $this->diafan->id, $this->diafan->element_type());
