...
echo '<section class="infoImg hedgeFond section-mg56">';

    echo '<div class="infoImg__inner container">';

        //вывод баннера в виде изображения
        if (! empty($result[0]['image']))
        {
            echo '<a class="infoImg__img" href="' .$result[0]['link'] .'">';
            echo '<img src="'.BASE_PATH.USERFILES.'/bs/'.$result[0]['image'].'" alt="'.(! empty($result[0]['alt']) ? $result[0]['alt'] : '').'" title="'.(! empty($result[0]['title']) ? $result[0]['title'] : '').'">';
            echo '</a>';
        }

        //вывод заголовка и описания баннера
        echo '<div class="infoImg__info">';
            if (! empty($result[0]['name']))
            {
                echo '<h2 class="infoImg__title">' .$result[0]['name'] .'</h2>';
            }
            ...
            echo '<a href="' .$result[0]['link'] .'" class="infoImg__link link-arrow">'.$this->diafan->_('more').'<SVG width="9" height="16" viewBox="0 0 9 16" fill="none" xmlns="http://www.w3.org/2000/svg">
