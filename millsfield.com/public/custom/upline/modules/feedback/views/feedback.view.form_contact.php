...
// {
// 	echo '<h3 class="modal__title"><span class="title_1">'.$result["name"].'</div></span><span class="title_2">' .$result["name"] .'</span></h3>';
// }

echo
'<form action="" id="form-3" class="form contact ajax" method="POST" enctype="multipart/form-data">
	...
	echo
'<H2>' .$this->diafan->_('title-contact-form') .'</H2>
<div class="discrip">' .$this->diafan->_('description-contact-form') .'</div>';
		...
		foreach ($result["rows"] as $row) //вывод полей из конструктора форм
		{
			...
			switch ($row["type"])
			{
				...
				case "phone":
					...
					<input class="required-field phone" type="text" name="p'.$row["id"].'" placeholder="' .$row["name"] . '" ' . ($row["required"] ? 'required="required "' : '') .'></div>';
					break;

				case "email":
					echo '<div class="form__field"><label>Email</label>
					<input class="required-field email" type="text" name="p'.$row["id"].'" placeholder="' .$row["name"] . '" ' . ($row["required"] ? 'required="required "' : '') .'></div>';
					break;
