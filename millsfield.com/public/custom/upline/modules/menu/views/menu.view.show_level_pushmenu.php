...
foreach ($result["rows"][$result["parent_id"]] as $row)
{
	if ($row["active"])
	{
		...
		echo '<li itemprop="name" class="nav__item nav__item-current active">';
	}
	elseif ($row["active_child"])
	{
		// начало пункта меню для активного дочернего пункта
		echo ' <li itemprop="name" class="nav__item parent active">';
	}
	elseif ($row["children"])
	{
		// начало пункта меню для элемента -родителя
		echo ' <li itemprop="name" class="nav__item parent">';
	}
	else
	{
		// начало любого другого пункта меню
		echo '<li itemprop="name" class="nav__item">';
	}
	...
	)
	{
		if ($row["othurl"])
		{
			echo '<a itemprop="url" href="'.$row["othurl"].'"'.$row["attributes"].''
			.(! empty($row["active"]) || ! empty($row["active_child"]) ? ' class="active"' : '')
			.'>';
		}
		else
		{
			echo '<a itemprop="url" href="'.BASE_PATH_HREF.$row["link"].'"'.$row["attributes"]
			.(! empty($row["active"]) || ! empty($row["active_child"]) ? ' class="active"' : '')
			.'>';
		}
	}
