...
class Session extends Diafan
{
	...
	public function write($key, $value)
	{
		...
		if(empty($row) || getenv('HTTP_USER_AGENT') != $row["user_agent"])
		{
			...
			{
				DB::query("INSERT INTO {sessions} (session_id, user_id, hostname, user_agent, referer, session, timestamp)"
				." VALUES ('%s', %d, '%s', '%s', '%h', '%s', %d)",
				$key, $this->diafan->_users->id, getenv("REMOTE_ADDR"), getenv('HTTP_USER_AGENT'), $referer, $value, time());
			}
		}
		else
		{
