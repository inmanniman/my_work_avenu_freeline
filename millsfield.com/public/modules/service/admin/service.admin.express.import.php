...
class Service_admin_express_import extends Frame_admin
{
	...
	private function show_import_upload_file()
	{
		...
		if(! $this->ftell || ! count($this->$files))
		{
			...
			echo'
					<div class="infofield">'.$this->diafan->_('Укажите формат файла').' <i class="tooltip fa fa-question-circle" title="'.$this->diafan->_('Поддерживаются следующие форматы данных: %s.', implode(", ", $this->diafan->array_column($this->extensions, "title"))).' '.$this->diafan->_('Файл желательно упаковать в ZIP-архив.').'"></i></div>';
