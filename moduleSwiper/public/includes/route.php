...
class Route extends Diafan
{
	 ...
	 * @param array $data данные, использующие для генерирования ЧПУ по шаблону
	 * @return void
	 */
	public function save($rewrite, $text, $element_id, $module_name, $element_type, $site_id, $cat_id = 0,  $parent_id = 0, $add_parents = false, $change_children = false, $data = array())
	{
		$text = trim($text);
		...
		else
		{
			...
			if($add_parents || ! $rewrite && ROUTE_AUTO_MODULE)
			{
				...
				{
					$rewrite_parent_site = DB::query_result("SELECT rewrite FROM {rewrite} WHERE module_name='site' AND element_id=%d AND element_type='element' LIMIT 1", $site_id);
				}
			}
			...
			if(! $rewrite && ROUTE_AUTO_MODULE)
			{
				...
				if($element_type == 'element' && $module_name == 'site' && $this->diafan->configmodules('rewrite_tpl', $module_name))
				{
					$rewrite_tpl = str_replace(
						array('%name', '%parent/', '%parent', '%id'),
						array($rewrite, ($rewrite_parent ? $rewrite_parent.'/' : ''), $rewrite_parent, $element_id),
						$this->diafan->configmodules('rewrite_tpl', $module_name)
					);
				}
				elseif($element_type == 'element' && $this->diafan->configmodules('rewrite_tpl', $module_name))
				{
					$rewrite_tpl = str_replace(
						array(
							'%name',
						...
						array($rewrite, ($rewrite_parent ? $rewrite_parent.'/' : ''),
							$rewrite_parent, $element_id,
							(! empty($data["article"]) ? $this->generate_rewrite($data["article"]) : ''),
							(! empty($data["created"]) ? date("d", $data["created"]) : ''),
							(! empty($data["created"]) ? date("m", $data["created"]) : ''),
							(! empty($data["created"]) ? date("Y", $data["created"]) : ''),
						),
						$this->diafan->configmodules('rewrite_tpl', $module_name)
					);
				}
				elseif($element_type == 'brand' && $this->diafan->configmodules('rewrite_tpl_brand', $module_name))
				{
					$rewrite_tpl = str_replace(
						array('%name', '%id'),
						array($rewrite, $element_id),
						$this->diafan->configmodules('rewrite_tpl_brand', $module_name)
					);
				}
				elseif($element_type == 'cat' && $this->diafan->configmodules('rewrite_tpl_cat', $module_name))
				{
					$rewrite_tpl = str_replace(
						array('%name', '%parent/', '%parent', '%id'),
						array($rewrite, ($rewrite_parent ? $rewrite_parent.'/' : ''), $rewrite_parent, $element_id),
						$this->diafan->configmodules('rewrite_tpl_cat', $module_name)
					);
				}
			}
			if($rewrite_tpl)
			...
			if(! $rewrite_parent && $rewrite_parent_site)
			{
				$rewrite = $rewrite_parent_site.'/'.$rewrite;
			}
			if(! $rewrite)
			{
				$rewrite .= $element_id;
	...
	public function module($module_name, $route_end = true)
	{
		...
		if (! isset($this->cache[$key][$module_name]))
		{
			...
			{
				$this->cache[$key][$module_name] = $this->link($site_id, 0, $route_end ? '' : $module_name, 'element', $route_end);
			}
		}
	...
	public function search($rewrite, $arguments_in_url = false, $binary = true, $like = false)
	{
		...
		if ($row = DB::query_fetch_array("SELECT module_name, element_id, element_type, rewrite FROM {rewrite} WHERE trash='0' AND".($binary ? " BINARY" : "").(! $like ? " rewrite='%h'" : " rewrite LIKE '%h%%'")." LIMIT 1", $rewrite))
		{
			return $row;
		}
		return false;
