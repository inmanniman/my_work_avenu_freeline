...
/**
 * Swiper_admin_counter
 */
class Swiper_admin_counter extends Frame_admin
{
	/**
	...
	public function prepare_config()
	{
		...
		if ($this->diafan->configmodules("cat", "swiper"))
		{
			$this->diafan->categories = DB::query_fetch_all(
				"SELECT id, [name], parent_id FROM {swiper_category} WHERE trash='0'"
	...
	public function show()
	{
		...
		{
			$inner = " INNER JOIN {swiper_category_rel} AS r ON r.element_id=s.id";
			$where = " AND r.cat_id=".$this->diafan->filter($_GET, "integer", "filter_cat_id");
		}

		$this->diafan->_paginator->navlink = 'swiper/counter/';
		$this->diafan->_paginator->get_nav = $this->diafan->get_nav;
		$this->diafan->_paginator->nen     = DB::query_result("SELECT COUNT(DISTINCT s.id) FROM {swiper} AS s".$inner." WHERE s.trash='0'".$where);
		$result["links"] = $this->diafan->_paginator->get();

		$result["rows"] = DB::query_range_fetch_all("SELECT s.id, s.[name], s.[act], cn.count_view FROM {swiper} AS s
		LEFT JOIN {swiper_counter} AS cn ON cn.element_id=s.id
		".$inner."
	...
	{
		if(! $this->diafan->configmodules("counter", "swiper"))
		{
		...
		foreach ($result["rows"] as $row)
		{
			...
			<div class="item__in">
			<div><a href="'.BASE_PATH_HREF.'swiper/edit'.$row["id"].'/">'.($row["name"] ? $row["name"] : $row["id"]).'</a></div>
