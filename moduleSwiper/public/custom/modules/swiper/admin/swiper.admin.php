 ...
 * Swiper_admin
 */
class Swiper_admin extends Frame_admin
{
	/**
	...
	{
		if(! $this->diafan->configmodules("cat", "swiper", $this->diafan->_route->site))
		{
		...
		if(! $this->diafan->configmodules("page_show", "swiper", $this->diafan->_route->site))
		{
	...
	public function show_add()
	{
		...
		{
			echo '<div class="error">'.sprintf($this->diafan->_('В %sнастройках%s включено использование альбомов. Чтобы загрузить фотографию, создайте хотя бы один %sальбом%s.'), '<a href="'.BASE_PATH_HREF.'swiper/config/">', '</a>', '<a href="'.BASE_PATH_HREF.'swiper/category/'.( $this->diafan->_route->site ? 'site'.$this->diafan->_route->site.'/' : '' ).'">', '</a>').'</div>';
		}
		else
		{
			echo '
	...
	public function list_variable_image($row, $var)
	{
		if(! isset($this->cache["prepare"]["image"]))
		{
			...
			$vs = unserialize($this->diafan->configmodules("images_variations_element", "swiper"));
			if(isset($vs["vs"]))
			{
				$vs = $vs["vs"];
			}
			foreach($vs as $v)
			{
				if($v["name"] == 'medium')
				{
					$this->cache["folder_image"] = 'swiper/'.DB::query_result("SELECT folder FROM {images_variations} WHERE id=%d", $v["id"]);
				}
			}
		}
	...
	public function upload()
	{
		...
		if (! $this->diafan->_users->roles('edit', 'swiper'))
		{
		...
		if ($this->diafan->config('element'))
		{
			...
			{
				$this->diafan->_route->cat = DB::query_result("SELECT id FROM {swiper_category} WHERE [act]='1' AND trash='0'".($this->diafan->_route->site ? " AND site_id=".$this->diafan->_route->site : '')." LIMIT 1");
			}
			$names[] = 'cat_id';
		...
		{
			$this->diafan->_route->site = DB::query_result("SELECT site_id FROM {swiper_category} WHERE id=%d LIMIT 1", $this->diafan->_route->cat);
		}
		elseif (! $this->diafan->_route->site)
		{
			$this->diafan->_route->site = DB::query_result("SELECT id FROM {site} WHERE module_name='swiper' LIMIT 1");
		}
		$values[] = "'".$this->diafan->_route->site."'";
		foreach ($this->diafan->_languages->all as $lang)
		{
			if($this->diafan->configmodules("multiupload_act", 'swiper', $this->diafan->_route->site))
			{
				$names[] = 'act'.$lang["id"];
				$values[] = "'1'";
			}
		...
		foreach ($_FILES["images"]['name'] as $i => $name)
		{
			...
			DB::query("INSERT INTO {swiper} (".implode(',', $new_names).") VALUES (".implode(',', $new_values).")");
			$save = DB::query_result("SELECT MAX(id) FROM {swiper} WHERE [name]='%s'", $name);
			DB::query("UPDATE {swiper} SET sort=id WHERE id=%d", $save);

			if ($this->diafan->config('element') && $this->diafan->_route->cat)
			{
				DB::query("INSERT INTO {swiper_category_rel} (element_id, cat_id) VALUES (%d, %d)", $save, $this->diafan->_route->cat);
			}
			...
			try
			{
				$this->diafan->_images->upload($save, 'swiper', 'element', $this->diafan->_route->site, $_FILES["images"]['tmp_name'][$i], $new_name);
			}
			catch(Exception $e)
			{
	...
	{
		$this->diafan->del_or_trash_where("swiper_rel", "element_id IN (".implode(",", $del_ids).") OR rel_element_id IN (".implode(",", $del_ids).")");
		$this->diafan->del_or_trash_where("swiper_counter", "element_id IN (".implode(",", $del_ids).")");
	}
}
