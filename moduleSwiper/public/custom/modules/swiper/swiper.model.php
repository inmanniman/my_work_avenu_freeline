 ...
 * swiper_model
 */
class swiper_model extends Model
{
	/**
	...
	private function list_query_count($time)
	{
		$count = DB::query_result(
			"SELECT COUNT(DISTINCT e.id) FROM {swiper} AS e"
			.($this->diafan->configmodules('where_access_element') ? " LEFT JOIN {access} AS a ON a.element_id=e.id AND a.module_name='swiper' AND a.element_type='element'" : "")
	...
	private function list_query($time)
	{
		$rows = DB::query_range_fetch_all(
			"SELECT e.id, e.[name], e.[anons], e.timeedit, e.site_id FROM {swiper} AS e"
			.($this->diafan->configmodules('where_access_element') ? " LEFT JOIN {access} AS a ON a.element_id=e.id AND a.module_name='swiper' AND a.element_type='element'" : "")
			." WHERE e.[act]='1' AND e.trash='0' AND e.site_id=%d"
			." AND e.date_start<=%d AND (e.date_finish=0 OR e.date_finish>=%d)"
			.($this->diafan->configmodules('where_access_element') ? " AND (e.access='0' OR e.access='1' AND a.role_id=".$this->diafan->_users->role_id.")" : '')
	...
	public function first_page()
	{
		...
		if (! $this->result = $this->diafan->_cache->get($cache_meta, $this->diafan->_site->module))
		{
			...
			{
				$this->diafan->_route->prepare($row["site_id"], $row["id"], "swiper", "cat");
				if ($this->diafan->configmodules("images_cat") && $this->diafan->configmodules("list_img_cat"))
				{
					$this->diafan->_images->prepare($row["id"], 'swiper', 'cat');
				}
			}
			foreach ($this->result["categories"] as &$row)
			{
				...
				$children = $this->diafan->get_children($row["id"], "swiper_category");
				$children[] = $row["id"];
				...
				$row["link_all"] = $this->diafan->_route->link($row["site_id"], $row["id"], 'swiper', 'cat');

				if ($this->diafan->configmodules("images_cat") && $this->diafan->configmodules("list_img_cat"))
				{
					$row["img"] =
					$this->diafan->_images->get(
							'medium', $row["id"], 'swiper', 'cat',
	...
	{
		$count = DB::query_result(
		"SELECT COUNT(DISTINCT c.id) FROM {swiper_category} AS c"
		.($this->diafan->configmodules('where_access_cat') ? " LEFT JOIN {access} AS a ON a.element_id=c.id AND a.module_name='swiper' AND a.element_type='cat'" : "")
	...
	{
		$rows = DB::query_range_fetch_all(
		"SELECT c.id, c.[name], c.[anons], c.timeedit, c.site_id FROM {swiper_category} AS c"
		.($this->diafan->configmodules('where_access_cat') ? " LEFT JOIN {access} AS a ON a.element_id=c.id AND a.module_name='swiper' AND a.element_type='cat'" : "")
		." WHERE c.[act]='1' AND c.parent_id=0 AND c.trash='0' AND c.site_id=%d"
	...
	{
		$rows = DB::query_range_fetch_all(
		"SELECT e.id, e.[name], e.timeedit, e.[anons], e.site_id FROM {swiper} AS e"
		." INNER JOIN {swiper_category_rel} AS r ON e.id=r.element_id"
		.($this->diafan->configmodules('where_access_element') ? " LEFT JOIN {access} AS a ON a.element_id=e.id AND a.module_name='swiper' AND a.element_type='element'" : "")
	...
	public function list_category()
	{
		...
		if (! $this->result = $this->diafan->_cache->get($cache_meta, $this->diafan->_site->module))
		{
			...
			if (empty($row) || (! empty($row['access']) && ! $this->access($row['id'], 'swiper', 'cat')))
			{
			...
			if ($this->diafan->configmodules("children_elements"))
			{
				$cat_ids = $this->diafan->get_children($this->diafan->_route->cat, "swiper_category");
		...
		foreach ($this->result["breadcrumb"] as $k => &$b)
		{
			...
			$b["name"] = $this->diafan->_useradmin->get($b["name"], 'name', $b["id"], 'swiper_category', _LANG);
		}
	...
	private function list_category_query()
	{
		...
		$row = DB::query_fetch_array("SELECT id, [name], [anons], [anons_plus] ".$fields.", timeedit, [descr], [keywords], [canonical], sort, parent_id, [title_meta], access, theme, view, view_rows, [act], noindex FROM {swiper_category}"
	...
	private function list_category_elements_query_count($time, $cat_ids)
	{
		...
		"SELECT COUNT(DISTINCT e.id) FROM {swiper} AS e"
		.($this->diafan->configmodules('where_access_element') ? " LEFT JOIN {access} AS a ON a.element_id=e.id AND a.module_name='swiper' AND a.element_type='element'" : "")
		." INNER JOIN {swiper_category_rel} AS r ON e.id=r.element_id AND r.cat_id IN (%s)"
	...
	public function id()
	{
		...
		if (! empty($_SERVER["HTTP_X_REQUESTED_WITH"]) && strtolower($_SERVER["HTTP_X_REQUESTED_WITH"]) == 'xmlhttprequest' && ! empty($_POST["module_ajax"]) && $_POST["module_ajax"] == "swiper")
		{
			$this->result["ajax"] = true;
			$res = array(
				'text' => $this->diafan->_tpl->get('id', 'swiper', $this->result),
				'h1' => $this->result["name"]
			);
	...
	private function id_query($time)
	{
		...
		." [canonical], [title_meta], access, theme, view, [act], date_start, date_finish, noindex".$fields." FROM {swiper}"
		." WHERE id=%d AND trash='0' AND site_id=%d"
	...
	public function show_block($count, $site_ids, $cat_ids, $sort, $images_variation, $tag)
	{
		...
		$cache_meta = array(
			...
			"count"    => $count,
			"lang_id" => _LANG,
			"current"  => ($this->diafan->_site->module == 'swiper' && $this->diafan->_route->show ? $this->diafan->_route->show : ''),
			"images_variation" => $images_variation,
			"access" => ($this->diafan->configmodules('where_access_element', 'swiper') || $this->diafan->configmodules('where_access_cat', 'swiper') ? $this->diafan->_users->role_id : 0),
			"time"     => $time,
			"tag" => $tag,
		);

		if ($sort == "rand" || ! $result = $this->diafan->_cache->get($cache_meta, "swiper"))
		{
			$minus = array();
			$one_cat_id = count($cat_ids) == 1 && substr($cat_ids[0], 0, 1) !== '-' ? $cat_ids[0] : false;
			if(! $this->validate_attribute_site_cat('swiper', $site_ids, $cat_ids, $minus))
			{
				return false;
			}
			$inner = "";
			...
			if($cat_ids)
			{
				$inner = " INNER JOIN {swiper_category_rel} as r ON r.element_id=e.id"
			...
			if($tag)
			{
				...
				$inner .= " INNER JOIN {tags} AS t ON t.element_id=e.id AND t.element_type='element' AND t.module_name='swiper' AND t.tags_name_id=".$t["id"];
			}
			...
			if ($sort == "rand")
			{
				...
				"SELECT COUNT(DISTINCT e.id) FROM {swiper} as e"
				.$inner
				.($this->diafan->configmodules('where_access_element', 'swiper') ? " LEFT JOIN {access} AS a ON a.element_id=e.id AND a.module_name='swiper' AND a.element_type='element'" : "")
				." WHERE e.[act]='1' AND e.trash='0'"
				.$where
				." AND e.date_start<=%d AND (e.date_finish=0 OR e.date_finish>=%d)"
				.($this->diafan->_site->module == 'swiper' && $this->diafan->_route->show ? " AND e.id<>".$this->diafan->_route->show : '')
				.($this->diafan->configmodules('where_access_element', 'swiper') ? " AND (e.access='0' OR e.access='1' AND a.role_id=".$this->diafan->_users->role_id.")" : ''), $time, $time
				);
				$rands = array();
			...
			foreach ($rands as $rand)
			{
				...
				.$where
				." AND e.date_start<=%d AND (e.date_finish=0 OR e.date_finish>=%d)"
				.($this->diafan->configmodules('where_access_element', 'swiper') ? " AND (e.access='0' OR e.access='1' AND a.role_id=".$this->diafan->_users->role_id.")" : '')
				." GROUP BY e.id"
				.$order,
				$time, $time,
				$sort == "rand" ? $rand : 0,
			...
			{
				$cat = DB::query_fetch_array("SELECT [name], site_id, id, spaceBetween1, effects1 FROM {swiper_category} WHERE id=%d LIMIT 1", $cat_ids[0]);
				$result["spaceBetween1"] = $cat["spaceBetween1"];
				$result["effects1"] = $cat["effects1"];
				$result["name"] = $cat["name"];
				$result["link_all"] = $this->diafan->_route->link($cat["site_id"], $cat["id"], 'swiper', 'cat');
			...
			if ($sort != "rand")
			{
				$this->diafan->_cache->save($result, $cache_meta, "swiper");
			}
		}
	...
	public function show_block_rel($count, $images_variation)
	{
		...
		if (! $result = $this->diafan->_cache->get($cache_meta, "swiper"))
		{
			$result["rows"] = DB::query_range_fetch_all(
			"SELECT e.id, e.[name], e.[anons], e.timeedit, e.site_id FROM {swiper} AS e"
			." INNER JOIN {swiper_rel} AS r ON e.id=r.rel_element_id AND r.element_id=%d"
			.($this->diafan->configmodules("rel_two_sided", 'swiper') ? " OR e.id=r.element_id AND r.rel_element_id=".$this->diafan->_route->show : '')
			.($this->diafan->configmodules('where_access_element', 'swiper') ? " LEFT JOIN {access} AS a ON a.element_id=e.id AND a.module_name='swiper' AND a.element_type='element'" : "")
			." WHERE e.[act]='1' AND e.trash='0'"
			." AND e.date_start<=%d AND (e.date_finish=0 OR e.date_finish>=%d)"
			.($this->diafan->configmodules('where_access_element', 'swiper') ? " AND (e.access='0' OR e.access='1' AND a.role_id=".$this->diafan->_users->role_id.")" : '')
	 ...
	 * шаблонного тега <insert name="show_previous_next" module="swiper" [template="шаблон"]>:
	...
	{
		if ($this->diafan->_site->module != 'swiper' || ! $this->diafan->_route->show && ! $this->diafan->_route->cat)
		{
			return;
		}
		$time = mktime(23, 59, 0, date("m"), date("d"), date("Y"));
		...
		if (! $result = $this->diafan->_cache->get($cache_meta, "swiper"))
		{
			...
			{
				$row = DB::query_fetch_array("SELECT sort FROM {swiper} WHERE id=%d LIMIT 1", $this->diafan->_route->show);
				
				$previous = DB::query_fetch_array(
				"SELECT e.[name], e.id FROM {swiper} AS e"
				.($this->diafan->configmodules('where_access_element') ? " LEFT JOIN {access} AS a ON a.element_id=e.id AND a.module_name='swiper' AND a.element_type='element'" : "")
				." WHERE e.[act]='1' AND e.trash='0' AND e.site_id=%d"
				...
				if ($previous)
				{
					...
					$result["previous"]["link"] = $this->diafan->_route->link($this->diafan->_site->id, $previous["id"], "swiper");
				}
				if ($next)
				{
					$result["next"]["text"] = $next["name"];
					$result["next"]["id"] = $next["id"];
					$result["next"]["link"] = $this->diafan->_route->link($this->diafan->_site->id, $next["id"], "swiper");
				}
			}
			else
			{
				$row = DB::query_fetch_array("SELECT sort, parent_id FROM {swiper_category} WHERE id=%d LIMIT 1", $this->diafan->_route->cat);
				
				$previous = DB::query_fetch_array(
				"SELECT c.[name], c.id FROM {swiper_category} AS c"
				.($this->diafan->configmodules('where_access_cat') ? " LEFT JOIN {access} AS a ON a.element_id=c.id AND a.module_name='swiper' AND a.element_type='element'" : "")
				...
				if ($previous)
				{
					$result["previous"]["text"] = $previous["name"];
					$result["previous"]["id"]   = $previous["id"];
					$result["previous"]["link"] = $this->diafan->_route->link($this->diafan->_site->id, $previous["id"], "swiper", 'cat');
				}
				$next = DB::query_fetch_array(
				"SELECT c.[name], c.id FROM {swiper_category} AS c"
				.($this->diafan->configmodules('where_access_cat') ? " LEFT JOIN {access} AS a ON a.element_id=c.id AND a.module_name='swiper' AND a.element_type='element'" : "")
				...
				if ($next)
				{
					$result["next"]["text"] = $next["name"];
					$result["next"]["id"] = $next["id"];
					$result["next"]["link"] = $this->diafan->_route->link($this->diafan->_site->id, $next["id"], "swiper", 'cat');
				}
			}
			//сохранение кеша
			$this->diafan->_cache->save($result, $cache_meta, 'swiper');
	...
	public function elements(&$rows, $images_variation = 'medium')
	{
		...
		foreach ($rows as &$row)
		{
			if ($this->diafan->configmodules('page_show', 'swiper', $row["site_id"]))
			{
				$this->diafan->_route->prepare($row["site_id"], $row["id"], "swiper");
			}
			$this->diafan->_images->prepare($row["id"], "swiper");
		}
		foreach ($rows as &$row)
		{
			if (! $this->diafan->configmodules("cat", "swiper", $row["site_id"]))
			{
				$row["cat_id"] = 0;
			}
			...
			if ($this->diafan->configmodules('page_show', 'swiper', $row["site_id"]))
			{
				$row["link"] = $this->diafan->_route->link($row["site_id"], $row["id"], "swiper");
			}
			else
			{
				$row["link"] = 'large';
			...
			$images  = $this->diafan->_images->get(
					$images_variation, $row["id"], 'swiper', 'element',
					$row["site_id"], $row["name"], 0,
					1,
					$row["link"]
				);
	...
	private function get_children_category($parent_id, $time)
	{
		...
		"SELECT c.id, c.[name], c.[anons], c.site_id FROM {swiper_category} AS c"
		.($this->diafan->configmodules('where_access_cat') ? " LEFT JOIN {access} AS a ON a.element_id=c.id AND a.module_name='swiper' AND a.element_type='cat'" : "")
		...
		foreach ($children as &$child)
		{
			if ($this->diafan->configmodules("images_cat") && $this->diafan->configmodules("list_img_cat"))
			{
				$this->diafan->_images->prepare($child["id"], 'swiper', 'cat');
			}
			$this->diafan->_route->prepare($child["site_id"], $child["id"], "swiper", "cat");
		}
		foreach ($children as &$child)
		{
			$child["link"] = $this->diafan->_route->link($child["site_id"], $child["id"], 'swiper', 'cat');
			if ($this->diafan->configmodules("images_cat") && $this->diafan->configmodules("list_img_cat"))
			{
				$child["img"] = $this->diafan->_images->get(
						'medium', $child["id"], 'swiper', 'cat', $child["site_id"],
						$child["name"], 0, $this->diafan->configmodules("list_img_cat") == 1 ? 1 : 0,
						$child["link"]);
			}
			$child["rows"] = array();
			$chn = $this->diafan->get_children($child["id"], "swiper_category");
			$chn[] = $child["id"];
			if ($this->diafan->configmodules("children_elements"))
			{
				$cat_ids = $chn;
	...
	{
		$this->diafan->_rating->get($row["id"], 'swiper', 'cat');
	...
	{
		$row["name"] = $this->diafan->_useradmin->get($row["name"], 'name', $row["id"], 'swiper_category', _LANG);
		if(! empty($row["anons"]))
		{
			$row["anons"] = $this->diafan->_useradmin->get($this->diafan->_tpl->htmleditor($row["anons"]), 'anons', $row["id"], 'swiper_category', _LANG);
		}
		if(! empty($row["text"]))
		{
			$row["text"] = $this->diafan->_useradmin->get($this->diafan->_tpl->htmleditor($row["text"]), 'text', $row["id"], 'swiper_category', _LANG);
		}
		$row["rating"] = $this->diafan->_rating->get($row["id"], 'swiper', 'cat', (! empty($row["site_id"]) ? $row["site_id"] : 0));
		if(! empty($row["children"]))
		{
			foreach ($row["children"] as &$ch)
			{
	...
	{
		$this->diafan->_tags->prepare($row["id"], 'swiper');
		$this->diafan->_rating->prepare($row["id"], 'swiper');
	}
	...
	public function format_data_element(&$row)
	{
		if (! empty($row["name"]))
		{
			$row["name"] = $this->diafan->_useradmin->get($row["name"], 'name', $row["id"], 'swiper', _LANG);
		}
		if (! empty($row["text"]))
		{
			$row["text"] = $this->diafan->_useradmin->get($this->diafan->_tpl->htmleditor($row["text"]), 'text', $row["id"], 'swiper', _LANG);
		}
		if(! empty($row["anons"]))
		{
			$row["anons"] = $this->diafan->_useradmin->get($this->diafan->_tpl->htmleditor($row["anons"]), 'anons', $row["id"], 'swiper', _LANG);
		}

		$row["tags"] =  $this->diafan->_tags->get($row["id"], 'swiper', 'element', (! empty($row["site_id"]) ? $row["site_id"] : 0));
		$row["rating"] = $this->diafan->_rating->get($row["id"], 'swiper', 'element', (! empty($row["site_id"]) ? $row["site_id"] : 0));
	}
}
