...
class Save_admin extends Diafan
{
	...
	public function save()
	{
		...
		if ($this->diafan->config("config"))
		{
			...
			$config_new_ids = array();
			$config_del_ids = DB::query_fetch_value("SELECT id FROM {config} WHERE module_name='%s' AND site_id=%d AND (lang_id="._LANG." OR lang_id=0)", $this->diafan->_admin->module, $this->diafan->_route->site, 'id');
			...
			for ($q = 0; $q < count($this->value); $q++)
			{
				...
				{
					$config_id = DB::query("INSERT INTO {config} (name, module_name, value, site_id, lang_id) VALUES ('%h', '%h', ".$mask.", '%d', '%d')", $name, $this->diafan->_admin->module, $this->value[$q], $this->diafan->_route->site, ($this->diafan->variable_multilang($name) ? _LANG : 0));

					if ($config_id)
					{
						$config_new_ids[] = $config_id;
					}
					else
					{
						$config_del_ids = $config_new_ids;
						break;
					}
				}
			}
			...
			if ($config_del_ids)
			{
				DB::query("DELETE FROM {config} WHERE id IN (%s)", implode(',', $config_del_ids));
			}
