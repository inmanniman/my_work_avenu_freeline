...
class Filemanager_admin extends Frame_admin
{
		...
		if ($this->diafan->_users->roles('del'))
		{
			...
			<div class="action-box">
				<div class="action-unit">
					...
					<select name="group_action" class="jq-selectbox">
			...
			.'">'.$this->diafan->_('Удалить').'</option>
				...
				<button class="btn btn_blue btn_small btn_disabled group_actions_filemanager">'.$this->diafan->_('Применить').'</button>
			</div>
			</form>';
		}
	...
	private function template_tree($rows)
	{
		...
		foreach ($rows as $row)
		{
			...
			{
				$row_id = md5($row["path"]);
				echo '<div class="div-checkbox"><input type="checkbox" name="ids[]" value="'.$row["path"].'" id="ids'.$row_id.'"><label for="ids'.$row_id.'" class="checkbox"></label></div>';
			}
