 ...
 * поддержка старого формата до версии 7.0.0.4
 *
 * @package    DIAFAN.CMS
 * @author     diafan.ru
 * @version    7.0
...
// тогда нужно заменить строку 20 строкой 19
...
class Shop_yandex extends Diafan
{
	...
	private function get_offers()
	{
		...
		$images = DB::query_fetch_key_array("SELECT id, name, folder_num, element_id FROM {images} WHERE module_name='shop' AND element_type='element' AND param_id=0 AND trash='0' ORDER BY sort ASC", "element_id");
			...
			while(1==1)
			{
				...
				foreach ($rows as $row)
				{
					...
					if($prices = $this->diafan->_shop->price_get_all($row["id"], 0))
					{
						...
						{
							$prices[$p_i]["price_param"] = DB::query_fetch_key_value("SELECT param_id, param_value FROM {shop_price_param} WHERE price_id=%d AND trash='0'", $prices[$p_i]["price_id"], "param_id", "param_value");
						}
					}
					...
					if (! empty($prices) AND $available == 'true')
					{
						...
						foreach($prices as $p_i => $dummy)
						{
							...
							{
								foreach ($prices[$p_i]["price_param"] as $price_param_id => $price_param_value)
								{
									if ($price_param_value)
									{
										$price_param[] = 'p'.$price_param_id.'='.$price_param_value;
									}
								}
							}
							...
							if (empty($yandex["typePrefix"]) || empty($yandex["vendor"]) || empty($yandex["vendorCode"]) || empty($yandex["model"]))
							{
								...
								<offer id="'.base_convert($row["id"], 10, 35).'z'.base_convert(($p_i+1), 10, 35).'" available="'.$available.'"'.($bid ? ' bid="'.$bid.'"' : '').($group_id !== false ? ' group_id="'.$group_id.'"' : '').'>
									<url>'.$link.($price_param ? '?'.implode('&amp;', $price_param): '').'</url>
									<price>'.$price.'</price>';
									if($oldprice)
									{
										$text .= '
