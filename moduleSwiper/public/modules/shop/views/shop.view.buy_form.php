...
'<form class="offer-d js_shop_form ajax" method="post" action="">
	...
	'<div class="offer-d__actionbar js_shop_buy">';
		...
		echo
	'</div>';
	if (! empty($result["row"]["goods_set"]))
	{
		echo '<div class="offer-d__good-set">';
		foreach ($result["row"]["goods_set"] as $good_set)
		{
			echo
			'<a class="button-d button-d_dark button-d_wide" href="'.BASE_PATH_HREF.$good_set['link'].'">
				<span class="button-d__name">'.$this->diafan->_('Купить в составе набора').'</span>
			</a>';
