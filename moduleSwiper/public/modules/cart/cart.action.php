...
class Cart_action extends Action
{
	...
	public function recalc()
	{
		...
		foreach ($rows as $row)
		{
			...
			if (! empty($_POST['wishlist'.$row['id']]) && $this->diafan->_route->module('wishlist'))
			{
				...
				$wishlist = array();
				$wcount = $this->diafan->_wishlist->get($row['good_id'], false, false, 'count');
				$wishlist['count'] = $wcount ? $wcount + $new_count[$row['id']] : $new_count[$row['id']];
				$wishlist['is_file'] = DB::query_result("SELECT is_file FROM {shop} WHERE id=%d", $row['good_id']);
				$wparam = array();
				if ($row['param'])
				{
					$wparam = DB::query_fetch_key_value("SELECT id, param_id FROM {shop_param_select} WHERE id IN (%s)", $row['param'], 'param_id', 'id');
				}
				if ($werror = $this->diafan->_wishlist->set($wishlist, $row['good_id'], $wparam, $row['additional_cost']))
				{
					$this->result['errors'][0] = $werror;
					return;
				}
		...
		if ($in_wish)
		{
			...
			$wishlist_model = new Wishlist_model($this->diafan);

			$wishlist_block = $wishlist_model->show_block();
			$this->result['data']['.js_show_wishlist'] = $this->diafan->_tpl->get('info', 'wishlist', $wishlist_block);

			if ($this->diafan->_site->module == 'wishlist')
			{
				$wishlist_model->form();
				$this->result['data']['.wishlist_table'] = $this->diafan->_tpl->get('table', 'wishlist', $wishlist_model->result);
			}
		}
	...
	private function send_mails($order_id, $params, $payment)
	{
		...
		{
			$message = str_replace(array('%title', '%url', '%id', '%message', '%payment', '%fio'),
			   array(TITLE, BASE_URL, $order_id, $this->message_admin_param, $payment_name, $user_fio),
			   $this->diafan->configmodules("sms_message_admin", 'shop')
			);
		...
		if (! empty($user_phone) && $this->diafan->configmodules("sendsmsnew", 'shop'))
		{
			$message = str_replace(array('%title', '%url', '%id', '%message', '%payment', '%fio'),
			   array(TITLE, BASE_URL, $order_id, $this->message_admin_param, $payment_name, $user_fio),
			   $this->diafan->configmodules("sms_message_new", 'shop')
			);

			$this->diafan->_postman->message_add_sms($message, $user_phone);
		}
