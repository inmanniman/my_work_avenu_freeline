	...
	foreach ($result["rows"] as $row)
	{
		...
		'<div class="table-d__row js_cart_item">';
			...
			'<div class="cell-d cell-d_details details-d">';
				...
				if(! empty($row['elements_good_set']))
				{
					echo
					'<div class="detail-d detail-d_good-set">
						<span class="detail-d__name">'.$this->diafan->_('Набор').':</span>
						<strong class="detail-d__content">';
							foreach ($row['elements_good_set'] as $i => $element)
							{
								echo '<a href="'.BASE_PATH_HREF.$element['link'].'">'.$element['name'].'</a>';

								if ($i < count($row['elements_good_set']) - 1)
								{
									echo ', ';
								}
							}
			...
			echo
			'<div class="cell-d cell-d_wishlist">
				<span class="wheart-d" title="'.$this->diafan->_('В избранное', false).'">
					<input class="wheart-d__input" type="checkbox" id="wish'.$row["id"].'" name="wishlist'.$row["id"].'" value="1">
					<label class="wheart-d__icon icon-d far fa-heart js_cart_wishlist" for="wish'.$row["id"].'" data-confirm="'.$this->diafan->_('Вы действительно хотите переместить товар из корзины в избранное?', false).'"></label>
				</span>
			</div>';
