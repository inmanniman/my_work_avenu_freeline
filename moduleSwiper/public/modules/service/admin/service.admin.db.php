...
<i class="fa fa-warning"></i><b>'.$this->diafan->_('Внимание! Это раздел для веб-мастера! Вносите изменения, только если знаете, что делаете!').'</b><br>'.$this->diafan->_('Модуль для формирования и экспорта дампа БД при переносе текущего сайта на другой хостинг, либо при создании резервной копии. А также для импорта дампа БД при переносе информации на текущий сайт.').'</div>';
		...
		<form action="" enctype="multipart/form-data" method="post" class="box box_half box_height">
			...
			<div class="box__heading">'.$this->diafan->_('Импорт резервной копии').'</div>
		...
		<div class="box box_half box_height box_right">
			<div class="box__heading">'.$this->diafan->_('Экспорт резервной копии БД').'</div>

			<a href="'.BASE_PATH.'service/export/?'.rand(0, 999999).'" class="file-load">
				<i class="fa fa-file-code-o"></i>
				'.$this->diafan->_('Скачать файл .zip').'
			</a>
		</div>';
	}
