...
class Service_admin_express_import extends Frame_admin
{
	...
	public $extensions = array(
		...
		'yml_offers' => array(
			...
			'type' => 'yml',
			'disabled' => true,
			'link' => 'shop/config/#tabs-4',
			'link_title' => 'Нажмите, чтобы перейти в отдельный модуль импорта YML'
		),
		'xls' => array(
			'title' => 'Excel',
	...
	private function show_mode_express()
	{
		...
		<div class="tabs" id="mode_express">
			...
			<a href="'.$this->url.'fields/'.'" class="tabs__item">'.$this->diafan->_('Правила импорта/экспорта').'</a>
		</div>';
	}
		...
		if(! $this->ftell || ! count($this->$files))
		{
			...
			foreach($this->extensions as $key => $value)
			{
				...
				echo '
					<div class="block_item">
						<input type="radio" name="extensions" value="'.$key.'" id="input_extensions_'.$key.'" '.$checked_attr.$onchange.(! empty($value['disabled']) ? ' disabled="disabled"' : '').' />'
						.'<label for="input_extensions_'.$key.'"'.$class.'>';
						if (! empty($value['link']))
						{
							echo '<a href="'.BASE_PATH_HREF.$value['link'].'"'.(! empty($value['link_title']) ? ' title="'.str_replace('"', '&quot;', $this->diafan->_($value['link_title'], false)).'"' : '').'>';
						}
						echo ! empty($value["name"]) ? $value["name"] : strtoupper($key);
						if (! empty($value['link']))
						{
							echo '</a>';
						}
			...
			echo'
				<div class="unit" id="file">
				 <div class="infofield">'.$this->diafan->_('Укажите файл (csv, xls, zip)').' <i class="tooltip fa fa-question-circle" title="'
		...
		foreach($rs as $r)
		{
			...
			if($r->count())
			{
				foreach ($r as $k => $v)
				{
					...
					// 60 = 64(limit) - 4(col_)
					$k = substr(preg_replace('/[^a-z0-9_]+/', '', strtolower(str_replace('-', '_', $this->diafan->translit($k)))), 0, 60);
	...
	private function show_import_description()
	{
		...
		if($cats = DB::query_fetch_key("SELECT id, name, module_name, site_id, cat_id, menu_cat_id, type, delete_items, add_new_items, update_items, act_items, header, sub_delimiter, count_part FROM {%s_category} WHERE trash='0' AND module_name='%s' ORDER BY sort ASC, id ASC", 'service_express_fields', $modules[$cat]["module_name"], "id"))
		{
			...
			//$cat_id = array_key_exists($cat_id, $cats) ? $cat_id : key($cats); //если не назначено правило, принудительно выбирать первое
		}
		else $cat_id = 0;
		$cats = array(0 => 'Создать новое правило импорта') + $cats;
		...
		// fields_cat_id
		$key = "fields_cat_id"; $name = $this->diafan->_("Выберите правило импорта или создайте новое");
		$value = $cat_id;
		$help = "Выберите ранее созданное правило импорта/экспорта записей или используйте новое описание.";
		$help = ' <i class="tooltip fa fa-question-circle" title="'.str_replace('"', '&quot;', $this->diafan->_($help)).'"></i>'
...
/*		
		// defaultcol
		$key = "defaultcol"; $name = $this->diafan->_("Определять поля по умолчанию как");
		$help = "Какой выбор делать для неопределенных полей при первой загрузке импорта.";
		$help = ' <i class="tooltip fa fa-question-circle" title="'.str_replace('"', '&quot;', $this->diafan->_($help)).'"></i>';
		...
		echo '
				<div class="unit" id="'.$key.'">
						<div class="infofield">'.$name.$help.'</div>
						<select name="'.$key.'">';
			echo '<option value="empty" rel="0" selected>'.$this->diafan->_("Пропустить поле").'</option>';
			echo '<option value="param" rel="0">'.$this->diafan->_("Дополнительная характеристика").'</option>';
		...
		*/
		...
		$help = "Если Вы загружаете товары впервые, и то, что уже есть на сайте не нужно, следует отметить эту опцию для удобства. На сайт импортируются новые записи из файла, а все уже существующие товары или категории будут удалены, за исключением тех, что будут обновлены (определяется по идентификатору).";
		$help = ' <i class="tooltip fa fa-question-circle" title="'.str_replace('"', '&quot;', $this->diafan->_($help)).'"></i>';
		$disabled = false;
		$attr = $class = "";
		...
		$help = "Если в файле импорта первой строкой указаны названия столбцов, отметьте этот флажок.";
		$help = ' <i class="tooltip fa fa-question-circle" title="'.str_replace('"', '&quot;', $this->diafan->_($help)).'"></i>';
		$disabled = false;
		$attr = $class = "";
		$object->show_table_tr_checkbox($key, $name, $value, $help, $disabled, $attr, $class);
		...
		if(! empty($tables[self::TABLE_NAME]) && DB::query_result("SELECT id FROM {".self::TABLE_NAME."} WHERE trash='0' LIMIT 1"))
		{
			...
			echo '
				<p>'.$this->diafan->_('Для автоматического назначения полей назовите их так же в первой строке файла импорта').'</p>';
	...
	public function table_row($module_name, $cat_id = 0)
	{
		...
		if($fields = DB::query_fetch_all("SELECT * FROM {%s} WHERE trash='0' AND cat_id=%d ORDER BY sort ASC, id ASC", "service_express_fields", $cat_id))
		{
			foreach ($fields as $k => $val)
			{
				...
				*/
				...
				ob_start(); $object->table_variable_type($val["id"], '', '', $k); $module_contents = ob_get_contents(); ob_end_clean();
		...
		if(! empty($table_rows))
		{
			...
			foreach($rows as $name => $value)
			{
				...
				{
					$key = 0;
					ob_start(); $object->table_variable_type($key, $name_field, '', $k, $value[0]); $module_contents = ob_get_contents(); ob_end_clean();
					$type_default = $module_contents;
					if(! empty($type_default))
					{
						echo $type_default.' <i class="tooltip fa fa-gear" title="'.$this->diafan->_("Настройки поля").'"></i>';
					}
				}
				echo '
						</td>';
				...
				{
					$key = 0;
					ob_start(); $object->table_variable_name($key, $value[0]); $module_contents = ob_get_contents(); ob_end_clean();
					$name_default = $module_contents;
					ob_start(); $object->table_variable_params($key, $value[0]); $module_contents = ob_get_contents(); ob_end_clean();
	...
	private function show_import_init()
	{
		...
		echo '
			...
			<div class="box__warning">
				...
				'.$this->diafan->_('Рекомендуем перед импортом %sсоздать резервную копию базы данных%s, если на сайте есть информация. Перед началом импорта сохраните правило импорта.', '<a href="'.BASE_PATH_HREF.'service/db/'.'" target="_blank">', '</a>').'
			</div>';
		echo '
		...
		$cat_id = $this->diafan->filter($_GET, 'integer', 'cat', 0);
		echo '
			<button id="express_button" class="btn btn_blue btn_small" default="'.$this->diafan->_("Начать импорт").'" action="'.($cat_id>0?'import':'only_save').'">'.$this->diafan->_("Начать импорт").'</button>
			';
		//else 
		//{
		/*echo '
			<button id="express_save_button" class="btn btn_blue btn_small" action="only_save">'.$this->diafan->_("Сохранить правило импорта").'</button>
			';*/
		echo '
			<img class="spinner_express hide" id="spinner_express" src="'.BASE_PATH.'adm/img/loading.gif">';
		//}
		echo '</div>';
