...
<i class="fa fa-warning"></i><b>'.$this->diafan->_('Внимание! Это раздел для веб-разработчика! Используйте модуль, только если знаете, что делаете!').'</b><br>'.$this->diafan->_('Активирование тем может произвести необратимые изменения базы данных и модулей.').' '.$this->diafan->_('Настоятельно рекомендуем предварительно сделать резервную копию файлов и базы данных сайта.').' '.$this->diafan->_('Не забывайте, что файлы тем обладают приоритетом над оригинальными файлами системы. Также важна очередность активации тем.').' '.$this->diafan->_('Подробнее в <a href="https://www.diafan.ru/dokument/full-manual/sysmodules/themes/" title="Документация к DIAFAN.CMS">документации</a>.').'<br>'.$this->diafan->_('Кнопка «Сгенерировать тему автоматически» запустит сканирование всех файлов CMS, найдет и вынесет всё, что было изменено в отдельную папку «Темы», восстановив исходные файлы из /return/1.zip').'</div>';
	}
	...
	{
		echo '<p><a class="btn" href="?action=custom_generate">'.$this->diafan->_('Сгенерировать тему автоматически').'</a> ';
	...
	{
		$text = '<div class="text" title="'.$this->diafan->_('Модуль сравнит инструкции для создания демо-контента из файлов модулей с реальным содержимым в БД и создаст новые инструкции для файла темы. ВНИМАНИЕ! Может потребоваться много ресурсов и времени.').'">';
	...
	public function list_variable_download($row, $var)
	{
		...
		if (is_dir(ABSOLUTE_PATH.'custom/'.$row["name"]) && is_readable(ABSOLUTE_PATH.'custom/'.$row["name"]) && $dir = opendir(ABSOLUTE_PATH.'custom/'.$row["name"]))
		{
			while (($file = readdir($dir)) !== false)
			{
				if ($file != '.' && $file != '..')
				{
					$text .= '<a href="'.BASE_PATH.'custom/export/'.$row["id"].'/?'.$this->diafan->uid().'" title="'.$this->diafan->_('После Автогенерации темы и добавления контента в этом файле будет установочный шаблон для создания клона этого сайта.').'">'.$this->diafan->_('Скачать в Zip').'</a>';
					break;
				}
			}
			closedir($dir);
